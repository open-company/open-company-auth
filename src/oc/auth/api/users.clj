    ...
    (let [current-password (:current-password user-props)
      ...
      (if (and (lib-schema/valid? user-res/User updated-user)
               (or (nil? new-password) ; not attempting to change password
                   ...
                   (and (not (empty? current-password)) (user-res/password-match? current-password (:password-hash user))))) ; attempting to change the password with an old password set, checking that the old password match
